...
def get_gmailctl_init_info(host_ip="192.168.0.25"):
    ...
    subprocess.run(["ssh", "-i", "/home/will/.ssh/id_rsa_lab", f"will@{host_ip}", "sudo pkill -f gmailctl || true"], capture_output=True)
    subprocess.run(["ssh", "-i", "/home/will/.ssh/id_rsa_lab", f"will@{host_ip}", "sudo tmux kill-session -t gmailctl_auth || true"], capture_output=True)
    ...
    while time.time() - start_time < timeout:
        ...
        res = subprocess.run([
            "ssh", "-i", "/home/will/.ssh/id_rsa_lab", f"will@{host_ip}", 
            "sudo tmux capture-pane -t gmailctl_auth -p"
...
**2. SSH Tunnel Command (Run this on your workstation):**
{{{{{{
ssh -i ~/.ssh/id_rsa_lab -L {port}:localhost:{port} will@{host_ip}
}}}}}}
