...
Purpose:
    ...
    Uses Gemini (via REST API) to find all sports-related articles, summarizes them,
    and marks them all as read to purge them from the feed.
...
def process_with_llm(items):
    print(f"Analyzing {len(items)} articles with Gemini to find Sports content...")
...
Your job is to:
1. Identify all articles that are about sports (e.g., football, basketball, baseball, hockey, soccer, golf, tennis, athletes, sports teams).
2. Write a brief summary of all the sports news contained in these articles. Mention the key sports events or headlines.
3. Return a JSON structure exactly matching this format:

{{
  "sports_summary": "A cohesive summary of all the sports headlines found in the feed...",
  "sports_article_ids": ["id_1", "id_2", "id_3"] // The IDs of ONLY the sports articles
}}
...
def main():
    parser = argparse.ArgumentParser(description="Purge and summarize sports articles.")
    ...
    print("\n--- SPORTS SUMMARY ---")
    print(analysis.get("sports_summary", "No sports news found."))
    
    to_mark_read = analysis.get("sports_article_ids", [])
    if to_mark_read:
        mark_as_read(auth_token, to_mark_read)
    else:
        print("\nNo sports items found to mark as read.")
